...
echo -e "${BOLD}│  All THREE parts together: algorithm, key material, AND comment.     │${NC}"
echo -e "${BOLD}└──────────────────────────────────────────────────────────────────────┘${NC}"
echo ""
echo -e "${GREEN}$(cat "${SSH_KEY}.pub")${NC}"
echo ""
echo -e "${BOLD}The line above has three parts — all required:${NC}"
echo ""
echo "   ssh-ed25519        AAAAC3... (long base64 string)        $(whoami)@$(hostname -s)-..."
echo "   ^^^^^^^^^^^        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^   ^^^^^^^^^^^^^^^^^^^^^^^^^"
echo "   PART 1:            PART 2:                               PART 3:"
echo "   algorithm name     the actual key (long, don't shorten)  your label/comment"
echo ""
echo -e "${YELLOW}Copy ALL three parts as one single line.${NC}  Oracle Cloud rejects the key"
echo "if any part is missing — including the comment at the end. The comment isn't"
echo "decoration, it's part of the key identity."
echo ""
