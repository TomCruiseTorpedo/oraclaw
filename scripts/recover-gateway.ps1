...
ssh -o BatchMode=yes -o ConnectTimeout=5 $VmHost 'true' 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Error "Could not reach $VmHost (check Tailscale + VM status)."
    exit 1
}

Write-Host "[recover] target: $VmHost  user: ubuntu" -ForegroundColor Cyan
...
Write-Host "  1) Journal (last 5 min):"
Write-Host "       ssh $VmHost 'journalctl --user -u openclaw-gateway --since `"5 minutes ago`" --no-pager | tail -60'"
Write-Host ''
Write-Host "  2) Service status:"
Write-Host "       ssh $VmHost 'systemctl --user status openclaw-gateway --no-pager -l'"
Write-Host ''
