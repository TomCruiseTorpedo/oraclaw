...
OUTPUT=$(ssh -o ConnectTimeout=5 "$VM_HOST" 'openclaw devices list' 2>&1) || die "Could not reach $VM_HOST (check Tailscale + VM status)."
...
ssh "$VM_HOST" "openclaw devices approve '$REQ_ID'"
echo ""
