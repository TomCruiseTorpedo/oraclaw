...
- Models: every slug is prefixed `openrouter/` so they route through the OpenRouter plugin (one API key). Primary: `openrouter/nvidia/nemotron-3-super-120b-a12b:free`. 4 free fallbacks: `openrouter/google/gemma-4-31b-it:free`, `openrouter/qwen/qwen3-next-80b-a3b-instruct:free`, `openrouter/google/gemma-4-26b-a4b-it:free`, `openrouter/qwen/qwen3-coder:free`. (Previous primary `openrouter/inclusionai/ling-2.6-flash:free` was dropped when OpenRouter announced its deprecation.)
- Heartbeat model override: `openrouter/google/gemma-4-26b-a4b-it:free` (26B MoE, ~4B active — cheap, fast, tool-capable) + `lightContext: true`. Much smaller than the main chain, because the recurring background check-ins fire far more often than user-initiated work. Verify the slug is still on the free tier at https://openrouter.ai/collections/free-models any time you suspect a model has been retired.
- Heartbeat schedule: one cron job every 6 hours with `sessionTarget: "main"` — check-ins land in the Main chat by design.
