...
- Models: every slug is prefixed `openrouter/` so they route through the OpenRouter plugin (one API key). Primary: `openrouter/nvidia/nemotron-3-super-120b-a12b:free`. 4 free fallbacks: `openrouter/google/gemma-4-31b-it:free`, `openrouter/minimax/minimax-m2.5:free`, `openrouter/z-ai/glm-4.5-air:free`, `openrouter/qwen/qwen3-coder:free`. (Previous primary `openrouter/inclusionai/ling-2.6-flash:free` was dropped when OpenRouter announced its deprecation.)
- Heartbeat model override: `openrouter/z-ai/glm-4.5-air:free` (35B, tool-use reliable) + `lightContext: true`. Much smaller than the main chain, because the recurring background check-ins fire far more often than user-initiated work. Verify the slug is still on the free tier at https://openrouter.ai/collections/free-models any time you suspect a model has been retired.
